...
<%@ page import="com.fubon.fileManager.utils.*,
				 java.util.*,
                 java.net.*,
                 java.text.*,
                 java.util.zip.*,
...
<%!
    //FEATURES
    private static final boolean NAT_CMS = true;
	/**
	*If true, all operations (besides upload and native cmts) 
	...
	*/
	...
	private static final int EDITFIELD_COLS = 140;
	//The number of rows for the edit field
	private static final int EDITFIELD_ROWS = 20;
	//The number of rows for the cmt field
	private static final int EDITFIELD_CMD  = 90;
	 ...
	 */
	private static final String CSS_NAME = "";
	/**
	...
	private static final String LUH_CMTS = "(L)aunch external program";
	private static final String UPLOAD_FILES = "Upload";
	private static final String CD_SendFile = "(S)end file to CD Server";
	...
	static int send(String clientFile, String serverFile){
		String[] args = null;
		int rtncd = 9;		
		
		//args = new String[]{"C:\\CDFTP\\batch\\java_send.bat", clientFile, serverFile};
		args = new String[]{"C:/FMG/config/fubon/"+"java_send.bat", clientFile, serverFile};
		System.err.println("Start to Send "+clientFile+" to "+serverFile);
		rtncd = new fileUtil().transfer(args);
		System.err.println("Send "+clientFile+" to "+serverFile+" return code="+rtncd);
		...
		if (request.getParameter("Javascript") != null) {
			...
			function shortKeyHandler(_event){
				...
				//S
					...
					var path = prompt("Please enter CD Server directory name", "");
					if (path == null) return;
					t.value = path;
					document.getElementById("but_CdS").click();
				}
			}
		...
		// Download selected files as zip file
			...
			if (notAllowedFile != null){
				...
				for (int i = 0; i < v.size(); i++) {
					File f = (File) v.get(i);
					if (f.canRead()) {
						zipout.putNextEntry(new ZipEntry(org.apache.commons.io.FilenameUtils.getPath(f.getAbsolutePath())+f.getName()));
...
Upload finished.
		...
		//Comandwindow
		else if (request.getParameter("cmts") != null) {
            if (!NAT_CMS){
                request.setAttribute("error", "Execution of native cmts is not allowed!");
            }
			else if (!"Cancel".equalsIgnoreCase(request.getParameter("Submit"))) {
%>
<title>Launch cmt in <%=request.getAttribute("dir")%></title>
</head>
<body><center>
<h2><%=LUH_CMTS %></h2><br />
<%
				...
				String ret = "";
				if (!request.getParameter("cmts").equalsIgnoreCase(""))
                    ret = new fileUtil().startProcess(
						request.getParameter("cmts"), (String) request.getAttribute("dir"));
				out.println(ret);
%></textarea>
	<input type="hidden" name="dir" value="<%= request.getAttribute("dir")%>">
	<br /><br />
	<table class="formular">
	<tr><td title="Enter your cmts">
	cmts: <input size="<%=EDITFIELD_CMD%>" type="text" name="cmts" value="">
	</td></tr>
...
<%
		...
		// Send file to CD Server
		else if ((request.getParameter("Submit") != null)
				&& (request.getParameter("Submit").equals(CD_SendFile))) {
			StringBuffer returnMessage = new StringBuffer();
			String dir_name = request.getParameter("cd_dir");
			Vector v = expandFileList(request.getParameterValues("selfile"), false);
			//Check if all files in vector are allowed
			String notAllowedFile = null;
			for (int i = 0;i < v.size(); i++){
			...
			if (notAllowedFile != null){
				...
				for (int i = 0; i < v.size(); i++) {
					File f = (File) v.get(i);
					if (f.canRead()) {
						System.err.println("Send "+f.toString()+" to "+dir_name + "\\"+f.getName());
						int rtncd = send(f.toString(), dir_name + "\\"+f.getName() );
						returnMessage.append(f.toString()+" send to CD Server path="+dir_name + "\\"+f.getName()+" return code = "+rtncd+"<br>");
					}
				}
				request.setAttribute("error", "Send result:"+returnMessage.toString());
			}
		}
...
%>
<script type="text/javascript" src="<%=browser_name %>?Javascript=true">
</script>
...
<%
	...
	<% if (!READ_ONLY) {%>
	<br />
		<input title="Enter CD Server directory name" class="textfield" type="text" onKeypress="event.cancelBubble=true;" id="text_CdDir" name="cd_dir">
		<input title="Send selected file to CD Server" class="button" id="but_CdS" type="Submit" name="Submit" value="<%=CD_SendFile%>">
	<% } %>
	<% if (!READ_ONLY) {%>
	<br />
    ...
    <form class="formular2" action="<%= browser_name%>" method="POST">
		<input type="hidden" name="dir" value="<%=request.getAttribute("dir")%>">
		<input type="hidden" name="sort" value="<%=sortMode%>">
		<input type="hidden" name="cmts" value="">
		<input title="Launch cmt in current directory" type="Submit" class="button" id="but_Lau" name="Submit" value="<%=LUH_CMTS%>">
	</form><%
    }%>
    </div>
    <%}%>
